...
--And hey. If you're gonna steal this... make your own damn puzzle for christs' sake. I made mine by planning it out in MSPaint first, if that helps.

--[[

Normal Mode changes:
Firewall does less damage the further right you are (although half your health if you're on the far left, so don't just stand there for 15 seconds)
You become immune as soon as you start moving onto the finish line square
GB locations are not as random, they only have a few slots per puzzle they can spawn in
Orange laser moved right a row in puzzle 2
Blaster fire interval increased
Some slight changes overall to the puzzles' layouts. Less required green tiles. Probably easier?
...
if (GetGlobal("attackNum") == 1) then
	...
	--	available[i] = i
	--end
	--You just insert numbers into the available table. Top blasters go first, then side blasters (so here, 11 = top row, side blaster)
                     table.insert(available, 1)
	table.insert(available, 2)
                     table.insert(available, 5)
	table.insert(available, 7)
	table.insert(available, 9)
	table.insert(available, 14)

	Arena.ResizeImmediate(620,180)
...
elseif (GetGlobal("attackNum") == 2) then
	XOFFSET = 42
	YOFFSET = 245
	GRIDWIDTH = 4
	GRIDHEIGHT = 15
	...
	table.insert(available, 2)
	table.insert(available, 9)
	table.insert(available, 12)
	table.insert(available, 13)
	...
	--if (GetGlobal("finishedpuzzle") == true) then
		--GBDELAY = 3
		--table.insert(bots, { startX = XOFFSET + 2*40, startY = YOFFSET - (6-1)*40, endX = 171, endY = 50, time = ourTime, speed = 0, rotation = 180, color = ORANGE })
...
elseif (GetGlobal("attackNum") == 3) then
	XOFFSET = 100
	YOFFSET = 245
	GRIDWIDTH = 4
	GRIDHEIGHT = 12
	...
	table.insert(available, 5)
	table.insert(available, 6)
	table.insert(available, 7)
	table.insert(available, 8)
	table.insert(available, 9)
	table.insert(available, 10)
	table.insert(available, 11)
	table.insert(available, 12)
	
	Arena.ResizeImmediate(500,180)
...
function Update()
	...
	if (finished == false and timeLimit - (Time.time - waveStartTime) < 0) then
		...
		if (posX == 1) then bullet.SetVar('damage',32)
		else bullet.SetVar('damage',16 - posX) end
		table.insert(oneFrameBullets, bullet)
...
function OnHit(bullet)
	...
	if(puzzle[posY][posX] ~= "finish" and bullet.GetVar('tile') ~= 1) then
