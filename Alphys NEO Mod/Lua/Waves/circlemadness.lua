...
for j = 0, 320, 40 do
	table.insert(pmetts, oblib.Parasol.new({ simple = true, circleID = 1, rotation = j, rspeed = 50, startX = 320, startY = 185, time = ourTime, ammo = 99, delay = 7, firstShot = math.random()*10+1, flingDir = -1 }))
end
...
function Update()
	...
	if (GetGlobal("justTaunted") == true) then
		for i=#bolts,1,-1 do
			if (ourTime > bolts[i].time and circles[bolts[i].circleID].radius > 0) then bolts[i].Update()
			else
				bolts[i].bullet.MoveToAbs(-500,-500)
			end
