...
for i = 1.4,7,0.7 do
	...
	if (math.random() < 0.2) then
		table.insert(gbs, oblib.GasterBlaster.new({ lifetime = 2, startX = -30, startY = -30, endX = 156, endY = -42, time = ourTime+i, rotation = 270, size = THIN, laserDelay = 0.9, laserLength = 0.55, s1 = false, s2 = false }))
	else
	...
	if (math.random() < 0.2) then
		table.insert(gbs, oblib.GasterBlaster.new({ lifetime = 2, startX = -30, startY = 510, endX = -42, endY = 360, time = ourTime+i, rotation = 0, size = THIN, laserDelay = 0.9, laserLength = 0.55 }))
	else
